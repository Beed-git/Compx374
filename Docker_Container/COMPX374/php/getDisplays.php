<?php
  ...
  $query = "select * from Competition order by competition_id DESC";
	...
	if($result1 != FALSE)
	{
		...
		//Initialize the counter for the competitions
		$countCompetition = 0;
		
		//Repeat for each competition
		while($rowCompetition = $result1->fetch()) 
		{
			echo "<div class='competition-container'>";
      
      //Output the competition name
      echo "<h2>Competition Name: ".$rowCompetition['name']."</h2>";
      
      //Get all displays in that competition
      $query = "select * from Display where competition_id=".$rowCompetition['competition_id'];
        ...
        echo "<div class='competition-displays-container'>";
        echo "<p>Before the displays</p>";
        
		    //Repeat for each competition
		    while($rowDisplay = $result2->fetch()) 
		    {
        ...
        echo "</div>";
			...
			//Increment the counter
			$countCompetition = $countCompetition + 1;
      
      echo "</div>";
		}

		//Check if there have been no competitions yet
		if ($countCompetition == 0)
		{
			echo "<p>There are no competitions.</p>";
		}		
	}
	else
	{
