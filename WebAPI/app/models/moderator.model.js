...
const getModeratorById = async function(id) {
    const res = await query("select * from Moderator where moderator_id = " + id);
    if (res.length > 0) {
...
const getModeratorByUsername = async function(username) {
    const res = await query("select * from Moderator where username = \"" + username + "\"");
    if (res.length > 0) {
        return Moderator(res[0]);
    } else return null;
}
...
const getModeratorByEmail = async function(email) {
    const res = await query("select * from Moderator where email = \"" + email + "\"");
    if (res.length > 0) {
        return Moderator(res[0]);
    } else return null;
}
